...
2. 各キーポイントについて、受験者の回答を以下の基準で判定する：
   ...
   - partial（70%）: キーポイントの概念・方向性には触れているが、重要な詳細・数値・具体的な語句が不十分
   - missed（0%）: 全く言及がない、または内容が明らかに誤っている
3. 判定に基づき、そのキーポイントの文言をそのまま covered/partial/missed のいずれかに振り分ける。
4. スコア計算：重み w=3→15点満点, w=2→10点満点, w=1→5点満点。covered=100%, partial=70%, missed=0%。各キーポイントのスコアを合算し、総満点で割って60点換算する。そこにベース40点を加算して最終スコアとする（score = 40 + 獲得点×60/総満点）。

【判定ルール】
- 迷った場合は missed ではなく partial を選ぶ（概念・方向性に触れていれば部分点を与える）
- キーワードの言い換えが正確なら covered、方向性は合っているが不完全なら partial、的外れなら missed
